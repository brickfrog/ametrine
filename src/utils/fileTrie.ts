...
export class FileTrieNode<T extends FileTrieData = ContentDetails> {
  ...
  get displayName(): string {
    ...
    const fallbackName = this.fileSegmentHint ?? this.slugSegment ?? "";

    // If we're using a fallback name (folder without explicit title), capitalize it
    return (
      this.displayNameOverride ?? nonIndexTitle ?? toTitleCase(fallbackName)
    );
  }
